...
def get_args_parser():
    ...
    # normalized mean and std control.
    ...
    parser.add_argument('--sing', default='none', type = str, choices=['none', 'singbn', 'singbn', 'singgbn','singln'])
...
def main(args):
    if args.model.startswith('resnet'):
        if args.sing == 'singbn':
            import advresnet as advres
        elif args.sing == 'singgbn':
            if args.activation == 'relu':
                import advresnet_gbn as advres
            elif args.activation == 'gelu':
                import advresnet_gbn_gelu as advres
    else:
    ...
    dataset_train_1, args.nb_classes = build_dataset(is_train=True, args=args)
    args.aa = 'rand-m2-mstd0.5-inc1'
    dataset_train_2, args.nb_classes = build_dataset(is_train=True, args=args)
    args.aa = 'rand-m3-mstd0.5-inc1'
    dataset_train_3, args.nb_classes = build_dataset(is_train=True, args=args)
    args.aa = 'rand-m4-mstd0.5-inc1'
    dataset_train_4, args.nb_classes = build_dataset(is_train=True, args=args)
    args.aa = 'rand-m5-mstd0.5-inc1'
    dataset_train_5, args.nb_classes = build_dataset(is_train=True, args=args)
    args.aa = 'rand-m6-mstd0.5-inc1'
    dataset_train_6, args.nb_classes = build_dataset(is_train=True, args=args)
    args.aa = 'rand-m7-mstd0.5-inc1'
    dataset_train_7, args.nb_classes = build_dataset(is_train=True, args=args)
    args.aa = 'rand-m8-mstd0.5-inc1'
    dataset_train_8, args.nb_classes = build_dataset(is_train=True, args=args)
    args.aa = 'rand-m9-mstd0.5-inc1'
    dataset_train_9, args.nb_classes = build_dataset(is_train=True, args=args)
    ...
    if args.distributed:
        ...
        if args.repeated_aug:
            ...
            sampler_train_3 = torch.utils.data.DistributedSampler(
                dataset_train_3, num_replicas=num_tasks, rank=global_rank, shuffle=True
            )
            sampler_train_4 = torch.utils.data.DistributedSampler(
                dataset_train_4, num_replicas=num_tasks, rank=global_rank, shuffle=True
            )
            sampler_train_5 = torch.utils.data.DistributedSampler(
                dataset_train_5, num_replicas=num_tasks, rank=global_rank, shuffle=True
            )
            sampler_train_6 = torch.utils.data.DistributedSampler(
                dataset_train_6, num_replicas=num_tasks, rank=global_rank, shuffle=True
            )
            sampler_train_7 = torch.utils.data.DistributedSampler(
                dataset_train_7, num_replicas=num_tasks, rank=global_rank, shuffle=True
            )
            sampler_train_8 = torch.utils.data.DistributedSampler(
                dataset_train_8, num_replicas=num_tasks, rank=global_rank, shuffle=True
            )
            sampler_train_9 = torch.utils.data.DistributedSampler(
                dataset_train_9, num_replicas=num_tasks, rank=global_rank, shuffle=True
            )
...
#     loss_scaler = scaler.AccumulativeScaler()
...
#                         'model_ema': get_state_dict(model_ema),
        ...
        if args.sing != 'none':
            if epoch == args.epochs - 1 and args.attack_iter!=0:
                ...
                test_stats = evaluate(data_loader_val, model, device)
                print('Final Test_attacker_step_10', test_stats["acc1"])
        ...
        # if args.mix == 'none' and args.sing == 'none':
        if args.sing == 'none':
